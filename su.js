/* ==================================================
   ...
   - v1.5.3: (CURRENT) Restrukturisasi mutlak KODE PENUH. Penyelarasan ID HTML secara menyeluruh, integrasi Modern Alert, Modifikasi Evaluasi Siswa Global dan FAB Chat/WA.
   ================================================== */
...
enableIndexedDbPersistence(db).catch((err) => { console.warn("Peringatan PWA Offline:", err.code); });

// Fungsi Pop-up Modern Pengganti Alert Bawaan
function showModernAlert(title, message, type = 'error') {
    ...
    if(!titleEl || !msgEl || !iconEl || !modalEl) return alert(message); // Fallback jika HTML belum termuat
...
if(document.getElementById("userNameDisplay")) document.getElementById("userNameDisplay").innerText = myName || "Admin AEC";
if(document.getElementById("userIdDisplay")) document.getElementById("userIdDisplay").innerText = actUser || "admin";
...
if(document.getElementById("btnCycleTheme")) {
    document.getElementById("btnCycleTheme").onclick = () => { currentThemeIndex = (currentThemeIndex + 1) % 3; applyThemeVisuals(currentThemeIndex); };
}
...
window.lihatLogbookArsip = async function(schoolId) {
    ...
    if(!area || !konten) return;
    document.getElementById("judulArsipLogbook").innerText = "Logbook: " + schoolId; 
...
window.langsungKeSekolah = function(val) {
    ...
    if(!val) {
        ...
        new bootstrap.Tab(document.querySelector('button[data-bs-target="#sub-sekolah"]')).show(); 
...
if(document.getElementById("btnSimpanRoadmap")) {
    document.getElementById("btnSimpanRoadmap").onclick = async () => {
        ...
        if(!w || !j) return showModernAlert("Peringatan", "Waktu Target dan Judul Kegiatan wajib diisi!");
        try { await addDoc(collection(db, "roadmaps"), { waktu_target: w, judul: j, deskripsi: d, created_at: serverTimestamp() }); showModernAlert("Berhasil", "Roadmap Program Harian berhasil disimpan!", "success"); document.getElementById("rmWaktu").value = ""; document.getElementById("rmJudul").value = ""; document.getElementById("rmDesc").value = ""; } catch(e) { showModernAlert("Kesalahan", e.message); }
    };
}
...
if(document.getElementById("btnSaveSchool")) {
    document.getElementById("btnSaveSchool").onclick = async () => {
        const sid = document.getElementById("inputIdSchool").value.toLowerCase().trim().replace(/\s+/g, ''); if(!sid) return showModernAlert("Kesalahan", "Identitas Handle Sekolah wajib diisi!");
        ...
        try { await setDoc(doc(db, "schools", sid), { namaSekolah: document.getElementById('inputSekolah').value, totalHari: parseInt(document.getElementById('inputTotalHari').value) || 5, hariBerjalan: parseInt(document.getElementById('inputHariKe').value) || 0, masterKelas: document.getElementById('inputMasterKelas').value, jadwal: document.getElementById('inputJadwal').value, briefing: document.getElementById('inputBriefing').value, goal: document.getElementById('inputGoal').value, masterSiswa: document.getElementById('inputMasterSiswa').value, kurikulum: { vocab: getArr('inputVocab'), speaking: getArr('inputSpeaking'), grammar: getArr('inputGrammar'), practice: getArr('inputPractice') }, assignedMentors: selectedMentors, waktuUpdate: serverTimestamp(), status: 'aktif' }, {merge:true}); showModernAlert("Berhasil", "Konfigurasi profil sekolah berhasil diperbarui!", "success"); window.langsungKeSekolah(sid); } catch (e) { showModernAlert("Kesalahan", e.message); }
    };
}

if(document.getElementById("btnArsipSekolah")) {
    document.getElementById("btnArsipSekolah").onclick = async () => { if(currentSchoolId && currentSchoolId !== 'NEW') { if(confirm("Anda yakin ingin menyembunyikan dan mengarsipkan data sekolah ini?")) { await setDoc(doc(db, "schools", currentSchoolId), { status: 'archived' }, {merge:true}); showModernAlert("Berhasil", "Sekolah Resmi Diarsipkan.", "success"); window.langsungKeSekolah(""); } } };
}

if(document.getElementById('btnImportExcel')) {
    document.getElementById('btnImportExcel').onclick = function() {
        const file = document.getElementById('excelSiswa').files[0]; if(!file) return showModernAlert("Peringatan", "Silakan lampirkan format berkas Excel terlebih dahulu!");
        const reader = new FileReader(); reader.onload = function(e) {
            try { const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, {type: 'array'}); const jsonData = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]); let grouped = {}; jsonData.forEach(row => { let kls = row['Kelas']; let nama = row['Nama']; if(kls && nama) { if(!grouped[kls]) grouped[kls] = []; grouped[kls].push(nama); } }); let output = []; for(let k in grouped) { output.push(`${k}: ${grouped[k].join(', ')}`); } document.getElementById('inputMasterSiswa').value = output.join('\n'); showModernAlert("Berhasil", "Penguraian (Impor) Data Excel Berhasil!", "success"); } catch(error) { showModernAlert("Gagal", "Format Berkas Tidak Valid."); }
...
if(document.getElementById("btnExportPDF")) {
    document.getElementById("btnExportPDF").onclick = () => {
        if(!currentSchoolId || currentSchoolId==='NEW') return showModernAlert("Peringatan", "Harap pilih lokasi sekolah di bilah atas untuk dicetak laporannya!");
...
if(document.getElementById("btnKirimTugasWA")) {
    document.getElementById("btnKirimTugasWA").onclick = async () => { const i = document.getElementById("waInstruksi").value; if(!i) return showModernAlert("Peringatan", "Bilah instruksi tugas teks wajib diisi!"); await addDoc(collection(db, "tugas_wa"), { schoolId: currentSchoolId, targetKelas: document.getElementById("waTarget").value || "GLOBAL", linkGambar: document.getElementById("waGambar").value, instruksi: i, waktu: serverTimestamp() }); showModernAlert("Berhasil", "Instruksi Tugas WhatsApp Berhasil Direkam!", "success"); document.getElementById("waInstruksi").value = ""; document.getElementById("waGambar").value = ""; };
}
...
if(document.getElementById("btnSaveMateri")) {
    document.getElementById("btnSaveMateri").onclick = async () => { const j = document.getElementById("materiJudul").value.trim(); const k = document.getElementById("materiKelas").value; const l = document.getElementById("materiLink").value.trim(); if(!j || !l) return showModernAlert("Peringatan", "Penyematan Judul dan Tautan Modul wajib diisi!"); await addDoc(collection(db, "materials"), { judul: j, kelas: k, link: l, waktu: serverTimestamp() }); showModernAlert("Berhasil", "Materi berhasil diunggah ke awan!", "success"); document.getElementById("materiJudul").value = ""; document.getElementById("materiLink").value = ""; };
}
