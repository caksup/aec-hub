/* ==================================================
   ...
   - v1.5.3: (CURRENT) Restrukturisasi logika mengikuti pembaruan tata letak menu HTML terbaru. Menyambungkan pemicu klik tombol melayang dan mengkalkulasi ulang prestasi Top 10 Gabungan serta daftar logbook aktif vs arsip di dalam menu Overview.
   ================================================== */
...
onSnapshot(collection(db, "schools"), (snap) => { 
    ...
    // Tab Logbook Arsip pada Menu Overview
...
window.lihatLogbookArsip = async function(schoolId) {
    ...
    document.getElementById("judulArsipLogbook").innerText = "Logbook: " + schoolId; area.classList.remove("d-none"); konten.innerHTML = '<div class="text-center small text-muted">Memuat...</div>';
    try {
        ...
        let htmlLogs = ''; arrLogs.forEach(d => { htmlLogs += `<div class="p-2 border rounded mb-2 bg-white shadow-sm"><div class="fw-bold text-wa border-bottom pb-1 mb-1">${d.nama.toUpperCase()} <span class="badge bg-secondary rounded-pill float-end">${d.kelas}</span></div><div class="text-dark mt-1 text-xs">📖 Materi: ${d.materi?.join(', ')}</div></div>`; });
    ...
    globalAllSchools.forEach(s => {
        ...
        container.innerHTML += `
            <div class="col-12 col-md-6">
                <div class="card border-0 shadow-sm rounded-4 h-100 bg-white p-1" style="cursor: pointer; border-left: 4px solid var(--wa-primary) !important;" onclick="window.langsungKeSekolah('${s.id}')">
...
window.langsungKeSekolah = function(val) {
    ...
    if(!val) {
        document.getElementById("adminLogbookList").innerHTML = `<div class="alert alert-secondary small text-center">Pilih sekolah aktif untuk melihat laporan terperinci.</div>`;
        document.getElementById("leaderboardTutor").innerHTML = `<div class="text-muted small text-center">Silakan pilih sekolah untuk memuat peringkat kinerja.</div>`;
...
function muatDataSekolah(sid) {
    unsubSchool = onSnapshot(doc(db, "schools", sid), (docSnap) => {
        ...
        const sels = ['filterKelasHistori', 'waTarget'];
...
// SETUP ROADMAP PROGRAM (Tampil di Overview, Input di Setup)
document.getElementById("btnSimpanRoadmap").onclick = async () => {
    ...
    if(!w || !j) return alert("Peringatan: Waktu Target dan Judul Kegiatan wajib diisi!");
    try { await addDoc(collection(db, "roadmaps"), { waktu_target: w, judul: j, deskripsi: d, created_at: serverTimestamp() }); alert("Roadmap Program Harian berhasil disimpan!"); document.getElementById("rmWaktu").value = ""; document.getElementById("rmJudul").value = ""; document.getElementById("rmDesc").value = ""; } catch(e) { alert(e.message); }
};
...
onSnapshot(query(collection(db, "roadmaps")), (snap) => {
    ...
    let dataRoadmap = []; snap.forEach(d => { dataRoadmap.push({id: d.id, ...d.data()}); }); dataRoadmap.sort((a,b) => (a.created_at?.toMillis() || 0) - (b.created_at?.toMillis() || 0)); // Ascending
...
window.hapusRoadmap = async function(id) { if(confirm("Hapus rincian peta jalan (roadmap) ini secara permanen?")) await deleteDoc(doc(db, "roadmaps", id)); };

// MANAJEMEN TUTOR (Penguncian Akun Admin)
...
// PENGATURAN SEKOLAH (SETUP SPESIFIK)
document.getElementById("btnSaveSchool").onclick = async () => {
    const sid = document.getElementById("inputIdSchool").value.toLowerCase().trim().replace(/\s+/g, ''); if(!sid) return alert("Identitas Handle Sekolah wajib diisi!");
    ...
    try { await setDoc(doc(db, "schools", sid), { namaSekolah: document.getElementById('inputSekolah').value, totalHari: parseInt(document.getElementById('inputTotalHari').value) || 5, hariBerjalan: parseInt(document.getElementById('inputHariKe').value) || 0, masterKelas: document.getElementById('inputMasterKelas').value, jadwal: document.getElementById('inputJadwal').value, briefing: document.getElementById('inputBriefing').value, goal: document.getElementById('inputGoal').value, masterSiswa: document.getElementById('inputMasterSiswa').value, kurikulum: { vocab: getArr('inputVocab'), speaking: getArr('inputSpeaking'), grammar: getArr('inputGrammar'), practice: getArr('inputPractice') }, assignedMentors: selectedMentors, waktuUpdate: serverTimestamp(), status: 'aktif' }, {merge:true}); alert("Konfigurasi profil sekolah berhasil diperbarui!"); window.langsungKeSekolah(sid); } catch (e) { alert(e.message); }
};
document.getElementById("btnArsipSekolah").onclick = async () => { if(currentSchoolId && currentSchoolId !== 'NEW') { if(confirm("Anda yakin ingin menyembunyikan dan mengarsipkan data sekolah ini?")) { await setDoc(doc(db, "schools", currentSchoolId), { status: 'archived' }, {merge:true}); alert("Sekolah Resmi Diarsipkan."); window.langsungKeSekolah(""); } } };
document.getElementById('btnImportExcel').onclick = function() {
    const file = document.getElementById('excelSiswa').files[0]; if(!file) return alert("Silakan lampirkan format berkas Excel terlebih dahulu!");
    const reader = new FileReader(); reader.onload = function(e) {
        try { const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, {type: 'array'}); const jsonData = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]); let grouped = {}; jsonData.forEach(row => { let kls = row['Kelas']; let nama = row['Nama']; if(kls && nama) { if(!grouped[kls]) grouped[kls] = []; grouped[kls].push(nama); } }); let output = []; for(let k in grouped) { output.push(`${k}: ${grouped[k].join(', ')}`); } document.getElementById('inputMasterSiswa').value = output.join('\n'); alert("Penguraian (Impor) Data Excel Berhasil!"); } catch(error) { alert("Format Berkas Tidak Valid."); }
    }; reader.readAsArrayBuffer(file);
};

// REKAPITULASI (LOGBOOK, KINERJA, PRESTASI TRACKER)
...
async function kalkulasiSiswaGlobal() {
    ...
    try {
        ...
        logbooksSnap.forEach(doc => {
            ...
            if(activeIds.includes(data.schoolId) && data.dataSiswa) {
                data.dataSiswa.forEach(siswa => {
                    ...
                    if(!rekapSiswaGlobal[identitas]) rekapSiswaGlobal[identitas] = { vocab: '-', speak: '-', grammar: '-', prac: '-', poinTotal: 0 };
                    let nilaiHuruf = (siswa.nilai || "").toUpperCase();
                    if(nilaiHuruf === 'A' || nilaiHuruf === 'B' || nilai洪'C') {
        ...
        // Peringkat Prestasi Tracker Global
        arrPrestasi.sort((a,b) => b.poin - a.poin).slice(0, 10);
        let presHtml = "";
        arrPrestasi.forEach((item, idx) => { let badge = idx === 0 ? "bg-warning text-dark" : (idx === 1 ? "bg-secondary text-white" : "bg-wa text-white"); presHtml += `<div class="d-flex justify-content-between align-items-center p-2 border rounded mb-1 bg-light text-sm"><div class="fw-bold text-dark"><span class="badge ${badge} me-2 rounded-pill">#${idx+1}</span> ${item.nama}</div><div class="fw-bold text-success">${item.poin} Pts</div></div>`; });
        trackPrestasi.innerHTML = presHtml || `<div class="text-center text-muted small p-3">Data prestasi kosong.</div>`;
...
document.getElementById("btnSaveMateri").onclick = async () => { const j = document.getElementById("materiJudul").value.trim(); const k = document.getElementById("materiKelas").value; const l = document.getElementById("materiLink").value.trim(); if(!j || !l) return alert("Penyematan Judul dan Tautan Modul wajib diisi!"); await addDoc(collection(db, "materials"), { judul: j, kelas: k, link: l, waktu: serverTimestamp() }); alert("Materi berhasil diunggah ke Pangkalan Data Awan!"); document.getElementById("materiJudul").value = ""; document.getElementById("materiLink").value = ""; };
onSnapshot(collection(db, "materials"), (snap) => { const list = document.getElementById("listGudangMateri"); if(!list) return; list.innerHTML = ""; snap.forEach(d => { const data = d.data(); list.innerHTML += `<div class="d-flex justify-content-between align-items-center p-2 border rounded mb-2 bg-white shadow-sm"><div><div class="fw-bold text-dark text-sm">${data.judul} <span class="badge bg-wa rounded-pill ms-1">${data.kelas}</span></div><a href="${data.link}" target="_blank" class="text-xs text-primary text-decoration-none"><i class="bi bi-link-45deg"></i> Buka Tautan Eksternal</a></div><button class="btn btn-sm btn-outline-danger py-0 px-2 rounded-pill" onclick="window.hapusModulMateri('${d.id}')" title="Hapus Modul"><i class="bi bi-trash"></i></button></div>`; }); });
