...
applyThemeVisuals(currentThemeIndex); // Set awal
...
function initGlobalUI() {
    ...
    <div class="modal fade" id="modalArsipSekolah" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer p-2 border-0"><button type="button" class="btn btn-secondary btn-sm rounded-pill fw-bold" data-bs-dismiss="modal">Tutup</button></div>
            </div>
        </div>
    </div>
    `;
    ...
    document.getElementById("btnSimpanRoadmap").onclick = async () => {
        ...
        try {
            ...
            alert("Roadmap berhasil disimpan!"); document.getElementById("rmWaktu").value = ""; document.getElementById("rmJudul").value = ""; document.getElementById("rmDesc").value = "";
    ...
    onSnapshot(query(collection(db, "roadmaps")), (snap) => {
        ...
        dataRoadmap.forEach(r => { 
            ...
            if(listSistem) listSistem.innerHTML += `<li class="list-group-item d-flex justify-content-between align-items-center"><div><div class="fw-bold">${r.judul}</div><div class="text-muted" style="font-size:0.65rem">${r.waktu_target}</div></div><i class="bi bi-trash text-danger" style="cursor:pointer;" onclick="window.hapusRoadmap('${r.id}')" title="Hapus"></i></li>`;
        });
        if(listOverview && listOverview.innerHTML === "") listOverview.innerHTML = `<li class="timeline-item"><div class="timeline-desc text-muted">Belum ada roadmap program.</div></li>`;
        if(listSistem && listSistem.innerHTML === "") listSistem.innerHTML = `<li class="list-group-item text-muted">Belum ada data.</li>`;
    });
}
...
function renderOverview() {
    ...
    if (globalAllSchools.length === 0) { container.innerHTML = `<div class="col-12"><div class="alert alert-light text-center small border shadow-sm text-muted">Belum ada sekolah aktif...</div></div>`; return; }
    globalAllSchools.forEach(s => {
        ...
        container.innerHTML += `<div class="col-12 col-md-6 col-lg-6"><div class="card border-0 shadow-sm rounded-4 h-100 bg-white cursor-pointer" onclick="window.langsungKeSekolah('${s.id}')" style="border-left: 4px solid var(--wa-primary) !important;"><div class="card-body p-3"><h6 class="fw-bold text-dark mb-3 text-truncate" title="${s.namaSekolah}"><i class="bi bi-building-check text-wa me-2"></i>${s.namaSekolah}</h6><div class="d-flex justify-content-between align-items-center mb-2"><span class="text-secondary text-xs fw-bold"><i class="bi bi-calendar-event me-1"></i> Hari ke:</span><span class="badge bg-wa text-white fw-bold px-2 py-1 rounded-pill">${hBerjalan} / ${tHari}</span></div><div class="d-flex justify-content-between align-items-center mb-2"><span class="text-secondary text-xs fw-bold"><i class="bi bi-mortarboard me-1"></i> Jumlah Siswa:</span><span class="badge bg-light text-dark fw-bold px-2 py-1 rounded-pill border">${jmlSiswa} Peserta</span></div><div class="d-flex justify-content-between align-items-center"><span class="text-secondary text-xs fw-bold"><i class="bi bi-person-video3 me-1"></i> Tutor Bertugas:</span><span class="badge bg-light text-dark fw-bold px-2 py-1 rounded-pill border">${jmlTutor} Tutor</span></div></div></div></div>`;
    });
}
...
function renderModernSchoolSelect() {
    ...
    document.querySelectorAll('.school-pill').forEach(btn => { btn.addEventListener('click', (e) => { window.langsungKeSekolah(e.target.getAttribute('data-value')); }); });
}

window.langsungKeSekolah = function(val) {
    bersihkanListener();
    if(!val) {
        currentSchoolId = ""; document.getElementById("adminLogbookList").innerHTML = `<div class="alert alert-secondary small text-center bg-transparent border">Pilih Sekolah di atas.</div>`;
        ...
        document.getElementById("leaderboardTutor").innerHTML = "Pilih sekolah dulu."; renderModernSchoolSelect(); 
        new bootstrap.Tab(document.querySelector('button[data-bs-target="#tab-overview"]')).show(); new bootstrap.Tab(document.querySelector('button[data-bs-target="#sub-cards"]')).show();
    } else if(val === "NEW") {
        ...
        currentAssignedMentors = []; renderMentorChecklist(); document.getElementById("adminLogbookList").innerHTML = `<div class="alert alert-warning small text-center">Simpan Handle School terlebih dahulu.</div>`;
        renderModernSchoolSelect(); new bootstrap.Tab(document.querySelector('button[data-bs-target="#tab-setup"]')).show();
    } else {
    ...
    unsubChats = onSnapshot(query(collection(db, "chats"), where("schoolId", "==", sid)), (snap) => {
        ...
        chats.forEach(c => {
            if (c.type === 'global') {
                ...
                let nameColor = isMe ? 'text-wa' : 'text-primary'; let roleBadge = ""; if(c.role === "admin") { nameColor = 'text-danger'; roleBadge = '<i class="bi bi-shield-lock-fill ms-1"></i>'; } else if(c.role === "direktur") { nameColor = 'text-warning'; roleBadge = '<i class="bi bi-star-fill ms-1 text-dark"></i>'; } 
...
document.getElementById('btnImportExcel').addEventListener('click', function() {
    ...
    reader.onload = function(e) {
        ...
        } catch(error) { alert("Gagal membaca Excel."); }
...
function renderListAdmin() {
    ...
    dataTampil.forEach(d => {
        const w = d.waktu ? d.waktu.toDate().toLocaleString('id-ID', {dateStyle:'short', timeStyle:'short'}) : '-'; let txtNilai = d.dataSiswa && d.dataSiswa.length > 0 ? d.dataSiswa.map(s => { let ikon = s.kehadiran === 'h' ? '✔' : (s.kehadiran === 'a' ? '✖' : s.kehadiran.toUpperCase()); return `<b>${s.nama}</b>(${ikon}, ${s.nilai||'-'})`; }).join(' | ') : 'Belum ada data'; let displayJam = (d.jamKe || "").replace('Jam ', 'Sesi '); let displayMateri = d.materi && d.materi.length > 0 ? d.materi.join(', ') : '-';
...
document.getElementById("btnKirimTugasWA").onclick = async () => { if(!currentSchoolId || currentSchoolId==='NEW') return alert("Pilih sekolah dulu!"); const t = document.getElementById("waTarget").value; const g = document.getElementById("waGambar").value; const i = document.getElementById("waInstruksi").value; if(!i) return alert("Instruksi wajib!"); await addDoc(collection(db, "tugas_wa"), { schoolId: currentSchoolId, targetKelas: t, linkGambar: g, instruksi: i, waktu: serverTimestamp() }); alert("Tugas WA Dibuat!"); document.getElementById("waInstruksi").value = ""; document.getElementById("waGambar").value = ""; new bootstrap.Tab(document.querySelector('button[data-bs-target="#sub-wa-hari"]')).show(); };
function renderTugasWA() { const listHariIni = document.getElementById("listTugasWAHarian"); const listSemua = document.getElementById("listTugasWAHistory"); const fil = document.getElementById("filterWA").value; listHariIni.innerHTML = ""; listSemua.innerHTML = ""; let dt = masterTugasWA; if(fil !== "SEMUA") dt = dt.filter(d => d.targetKelas === fil); let currentHariValue = document.getElementById("filterHari").options.length > 1 ? document.getElementById("filterHari").options[document.getElementById("filterHari").options.length-1].value : "SEMUA"; let htmlSemua = ""; let htmlHarian = ""; dt.forEach((d, i) => { const w = d.waktu ? d.waktu.toDate().toLocaleDateString('id-ID', { dateStyle: 'long'}) : 'Baru Saja'; let imgTag = d.linkGambar ? `<img src="${d.linkGambar}" class="img-fluid rounded mb-2 border w-100" style="max-height: 200px; object-fit: cover;">` : ''; let itemHtml = `<div class="card card-custom p-3 mb-3 bg-white"><div class="d-flex justify-content-between mb-2"><span class="badge bg-wa rounded-pill">${d.targetKelas}</span><span class="text-xs text-muted">${w}</span></div>${imgTag}<div class="p-2 bg-light border rounded text-sm mb-2 font-monospace" id="txtWA_${i}" style="white-space: pre-line;">${d.instruksi}</div><div class="d-flex gap-2"><button class="btn btn-wa btn-sm flex-fill fw-bold rounded-pill" onclick="window.kirimKeWA('${encodeURIComponent(d.instruksi)}')"><i class="bi bi-whatsapp me-1"></i> Kirim ke WA</button><button class="btn btn-outline-danger btn-sm rounded-pill" onclick="window.aksidminHapusTugas('${d.id}')"><i class="bi bi-trash"></i></button></div></div>`; htmlSemua += itemHtml; if(d.waktu && d.waktu.toDate().toLocaleDateString('id-ID') === currentHariValue) htmlHarian += itemHtml; }); listSemua.innerHTML = htmlSemua || `<div class="alert text-center border-0 text-muted bg-transparent mt-3">Kosong.</div>`; listHariIni.innerHTML = htmlHarian || `<div class="alert text-center border-0 text-muted bg-transparent mt-3">Kosong. Cek tab History.</div>`; } 
window.kirimKeWA = function(encodedText) { window.open(`https://wa.me/?text=${encodedText}`, '_blank'); }

document.getElementById("btnSendChat").onclick = async () => { if(!currentSchoolId || currentSchoolId==='NEW') return alert("Pilih sekolah dulu!"); const msg = document.getElementById("inputChat").value.trim(); if(!msg) return; await addDoc(collection(db, "chats"), { schoolId: currentSchoolId, sender: myName, message: msg, waktu: serverTimestamp(), type: 'global', role: "admin" }); document.getElementById("inputChat").value = ""; }; 
...
function renderMentorChecklist() { const setupMentorList = document.getElementById("setupMentorList"); if (!setupMentorList) return; setupMentorList.innerHTML = ""; const mentorsAktif = globalAllUsers.filter(u => u.status === 'aktif'); if(mentorsAktif.length === 0) { setupMentorList.innerHTML = "<span class='text-muted'>Belum ada akun mentor aktif...</span>"; return; } mentorsAktif.forEach(u => { const isChecked = currentAssignedMentors.includes(u.id) ? "checked" : ""; setupMentorList.innerHTML += `<div class="col-6 col-md-4"><div class="form-check border p-1 rounded bg-white shadow-sm"><input class="form-check-input check-mentor ms-1" type="checkbox" value="${u.id}" id="chk_${u.id}" ${isChecked}><label class="form-check-label fw-bold ms-1 text-dark" style="font-size:0.8rem; cursor:pointer;" for="chk_${u.id}">${u.julukan}</label></div></div>`; }); }
...
window.siapEditUser = function(uid, nama, role, pin) { document.getElementById("newUser").value = uid; document.getElementById("newUser").readOnly = true; document.getElementById("newPin").value = pin; document.getElementById("newName").value = nama; document.getElementById("newRole").value = role; const btn = document.getElementById("btnAddUser"); btn.innerText = "Update Akun"; btn.classList.replace("btn-wa", "btn-warning"); if(!document.getElementById("btnBatalEditUser")) { const btnBatal = document.createElement("button"); btnBatal.id = "btnBatalEditUser"; btnBatal.className = "btn btn-sm btn-secondary w-100 mt-2 fw-bold rounded-pill"; btnBatal.innerText = "Batal Edit"; btnBatal.onclick = window.batalEditUser; btn.parentNode.appendChild(btnBatal); } else { document.getElementById("btnBatalEditUser").style.display = "block"; } };
window.batalEditUser = function() { document.getElementById("newUser").value = ""; document.getElementById("newUser").readOnly = false; document.getElementById("newPin").value = ""; document.getElementById("newName").value = ""; document.getElementById("newRole").value = "mentor"; const btn = document.getElementById("btnAddUser"); btn.innerText = "Simpan Akun"; btn.classList.replace("btn-warning", "btn-wa"); if(document.getElementById("btnBatalEditUser")) document.getElementById("btnBatalEditUser").style.display = "none"; };
...
document.getElementById("btnAddUser").onclick = async () => { const u = document.getElementById("newUser").value.toLowerCase().trim(); const p = document.getElementById("newPin").value; const n = document.getElementById("newName").value; const r = document.getElementById("newRole").value; if(!u || !p || !n) return alert("Isi form dhisik!"); await setDoc(doc(db, "users", u), { pin: p, julukan: n, role: r, status: "aktif" }, { merge: true }); alert("Data Akun Disimpen!"); window.batalEditUser(); }; 
document.getElementById("btnSaveMateri").onclick = async () => { const j = document.getElementById("materiJudul").value; const k = document.getElementById("materiKelas").value; const l = document.getElementById("materiLink").value; if(!j || !l) return alert("Judul dan Link wajib diisi!"); await addDoc(collection(db, "materials"), { judul: j, kelas: k, link: l, waktu: serverTimestamp() }); alert("Materi tersimpan!"); document.getElementById("materiJudul").value = ""; document.getElementById("materiLink").value = ""; };
onSnapshot(collection(db, "materials"), (snap) => { const list = document.getElementById("listGudangMateri"); list.innerHTML = ""; let html = ""; snap.forEach(d => { const data = d.data(); const id = d.id; html += `<div class="d-flex justify-content-between align-items-center p-2 border rounded mb-2 bg-white shadow-sm"><div><div class="fw-bold text-dark text-sm">${data.judul} <span class="badge bg-wa rounded-pill ms-1">${data.kelas}</span></div><a href="${data.link}" target="_blank" class="text-xs text-primary text-decoration-none"><i class="bi bi-link-45deg"></i> Buka Link</a></div><button class="btn btn-sm btn-outline-danger py-0 px-2 rounded-pill" onclick="window.hapusMateri('${id}')"><i class="bi bi-trash"></i></button></div>`; }); list.innerHTML = html || "<div class='text-muted text-center small mt-2'>Belum ada materi.</div>"; });
...
document.getElementById("btnExportPDF").onclick = () => {
    ...
    let htmlString = `<h3 style="text-align:center; font-family: Arial, sans-serif; margin-bottom: 20px;">LAPORAN LOGBOOK MENTOR<br><small style="font-size:12px; font-weight:normal;">Sekolah: ${document.getElementById('inputSekolah').value} | Filter: ${fHari}</small></h3><table style="width: 100%; border-collapse: collapse; font-size: 10px; font-family: Arial, sans-serif;" border="1"><tr style="background-color: #f2f2f2; text-align: center;"><th style="padding:5px;">WAKTU</th><th style="padding:5px;">MENTOR</th><th style="padding:5px;">KELAS / JAM</th><th style="padding:5px; width: 35%;">MATERI & LAPORAN</th><th style="padding:5px; width: 35%;">RAPOR SISWA</th></tr>`;
    dataTampil.forEach(d => {
