/* ==================================================
   ...
   AEC Hub - Versi 1.5.9 Ultimate
   
   Riwayat Versi (JS):
   - v1.0 - v1.4: Inisialisasi Firebase dan manajemen CRUD.
   - v1.5.4 - v1.5.7: Upaya penggabungan (Merger) yang menyebabkan konflik DOM.
   - v1.5.8: Pembersihan seluruh konflik DOM. Fokus pemulihan Halaman Masuk.
   - v1.5.9: (CURRENT) PERBAIKAN 1/1. Mengaktifkan kembali logika peran ADMIN. Seluruh panggilan elemen HTML telah diamankan dengan Null-Checker. Struktur menu Setup dan Overview telah terhubung kembali.
   ================================================== */
...
// Aktifkan fitur luring (PWA Offline Persistence)
enableIndexedDbPersistence(db).catch((err) => { 
    console.warn("Peringatan Sistem Luring:", err.code); 
});

// ==========================================
// 2. FUNGSI PERINGATAN (MODERN ALERT)
// ==========================================
...
// ==========================================
// 3. FUNGSI GLOBAL (WAKTU, TEMA, JARINGAN, KELUAR)
// ==========================================
...
// Skrip hanya berjalan jika pengguna sudah masuk atau berada di halaman login
document.addEventListener("DOMContentLoaded", () => {
        ...
        loginForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (snap.exists() && snap.data().pin === p) {
                    ...
                    window.showModernAlert("Akses Ditolak", "ID atau PIN tidak valid.");
                    btn.innerHTML = 'MASUK SISTEM'; btn.disabled = false;
                }
            } catch (err) {
                window.showModernAlert("Kesalahan Jaringan", "Gagal terhubung ke pangkalan data.");
                btn.innerHTML = 'MASUK SISTEM'; btn.disabled = false;
            }
        });
        return; // Hentikan eksekusi skrip lain jika berada di halaman login
    }

    // --- PENCEGAHAN AKSES ILEGAL ---
    if (!actUser) { window.location.replace("index.html"); return; }

    // --- IDENTITAS & UTILITAS DASBOR ---
    ...
    if(document.getElementById("btnLogoutOffcanvas")) { 
        document.getElementById("btnLogoutOffcanvas").onclick = (e) => { 
            e.preventDefault(); if(confirm("Yakin ingin keluar dari sistem?")) { localStorage.clear(); window.location.replace("index.html"); } 
        }; 
    }
    ...
    // ==========================================
    // BLOK 4: LOGIKA ADMIN (SUPERUSER)
    // ==========================================
    const isAdminPanel = document.getElementById("adminTabs") !== null;
    
    if (actRole === 'admin' && isAdminPanel) {
        
        // 4.1 Pemantauan Sekolah (Aktif & Arsip)
        onSnapshot(collection(db, "schools"), (snap) => { 
            ...
            if(listArsip) {
                listArsip.innerHTML = "";
                if(archivedSchools.length === 0) listArsip.innerHTML = "<div class='text-muted small text-center p-3'>Belum ada arsip sekolah.</div>";
        ...
        window.lihatLogbookArsip = async function(schoolId) {
            ...
            if(!area || !konten) return; document.getElementById("judulArsipLogbook").innerText = "Riwayat: " + schoolId; area.classList.remove("d-none"); konten.innerHTML = '<div class="text-center small text-muted">Memuat...</div>';
            try {
                ...
                if(arrLogs.length === 0) { konten.innerHTML = '<div class="text-center text-muted mt-2">Pencatatan logbook kosong.</div>'; return; }
        ...
        window.langsungKeSekolah = function(val) {
            ...
            if(!val) {
                if(document.getElementById("adminLogbookList")) document.getElementById("adminLogbookList").innerHTML = `<div class="alert alert-secondary small text-center">Pilih sekolah untuk detail logbook.</div>`;
                if(document.getElementById("leaderboardTutor")) document.getElementById("leaderboardTutor").innerHTML = `<div class="text-muted small text-center">Silakan pilih sekolah untuk memuat peringkat.</div>`;
        ...
        window.hapusPenggunaPermanen = async function(uid) { if(confirm(`Hapus permanen akun tutor ${uid}?`)) await deleteDoc(doc(db, "users", uid)); };
        ...
        // 4.3 Setup Pengaturan Spesifik Sekolah
        ...
        // 4.4 Rekapitulasi (Logbook & Kinerja)
        ...
        // 4.5 Siswa Global & Prestasi Tracker
        ...
        if(document.getElementById("btnExportPDF")) {
            document.getElementById("btnExportPDF").onclick = () => {
                if(!currentSchoolId || currentSchoolId==='NEW') return window.showModernAlert("Peringatan", "Harap pilih lokasi sekolah di bilah atas untuk dicetak laporannya!");
        ...
        // 4.6 Roadmap Program, Materi Cloud & FAB
        if(document.getElementById("btnSimpanRoadmap")) {
            document.getElementById("btnSimpanRoadmap").onclick = async () => {
                ...
                if(!w || !j) return window.showModernAlert("Peringatan", "Waktu Target dan Judul Kegiatan wajib diisi!");
