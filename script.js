/* ==================================================
   ...
   AEC Hub - Versi 1.6.7 Ultimate
   
   Riwayat Versi (JS):
   - v1.0 - v1.6.6: Penggabungan (Merger) & Perbaikan Routing DOM.
   - v1.6.7: (CURRENT) FIX TOTAL. Pemulihan logika Fallback Login khusus untuk bypass Firebase saat database kosong. Migrasi perlindungan ke sessionStorage sesuai arsitektur awal.
   ================================================== */
...
enableIndexedDbPersistence(db).catch((err) => { console.warn("PWA Luring Error:", err.code); });

// ==========================================
// 1. MESIN POP-UP MODERN (TOAST/MODAL)
// ==========================================
window.showPopup = function(pesan, tipe = "error") {
    // Mode Toast (Halaman Index)
    const toastEl = document.getElementById('liveToast');
    const toastMsg = document.getElementById('toastMessage');
    
    // Mode Modal (Halaman Dasbor)
    ...
    if (toastEl && toastMsg) {
        ...
        if (tipe === "error") { toastEl.classList.add('text-bg-danger'); toastMsg.innerHTML = `<i class="bi bi-exclamation-octagon-fill me-2 fs-5 align-middle"></i> ${pesan}`; } 
        else if (tipe === "success") { toastEl.classList.add('text-bg-success'); toastMsg.innerHTML = `<i class="bi bi-check-circle-fill me-2 fs-5 align-middle"></i> ${pesan}`; } 
        else { toastEl.classList.add('text-bg-warning', 'text-dark'); toastMsg.classList.replace('text-white', 'text-dark'); toastMsg.innerHTML = `<i class="bi bi-info-circle-fill me-2 fs-5 align-middle"></i> ${pesan}`; }
        new bootstrap.Toast(toastEl, { delay: 3500 }).show();
    } else if (modalEl && tEl && mEl) {
        tEl.innerText = tipe === 'error' ? "Peringatan" : "Pemberitahuan"; mEl.innerText = pesan;
        if (tipe === 'error') iEl.className = 'bi bi-x-circle-fill text-danger mb-3 d-block'; 
        else if (tipe === 'success') iEl.className = 'bi bi-check-circle-fill text-success mb-3 d-block'; 
...
// ==========================================
// 2. FUNGSI AUTENTIKASI DENGAN FALLBACK
// ==========================================
...
// Sesi Menggunakan sessionStorage
const actUser = sessionStorage.getItem("loggedInUser"); 
const actRole = sessionStorage.getItem("userRole"); 
const myName = sessionStorage.getItem("loggedInName");

const isDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
document.documentElement.setAttribute('data-theme', isDark ? 'dark' : 'light');

// ==========================================
// 3. LOGIKA HALAMAN INDEX (LOGIN)
// ==========================================
...
if (loginForm) {
    ...
    loginForm.addEventListener("submit", async (e) => {
        ...
        if (!u || !p) return window.showPopup("Username dan PIN wajib diisi!", "error");
        
        btn.innerHTML = `<span class="spinner-border spinner-border-sm me-2"></span> Memproses...`; 
        btn.disabled = true;
        
        try {
            const userData = await verifyLogin(u, p);
            window.showPopup(`Login Sukses! Sugeng rawuh, ${userData.julukan}...`, "success");
            
            sessionStorage.setItem("loggedInUser", u);
            sessionStorage.setItem("loggedInName", userData.julukan);
            ...
            window.showPopup(error.message, "error"); 
            btn.innerHTML = `MASUK SISTEM <i class="bi bi-box-arrow-in-right ms-2"></i>`; btn.disabled = false; 
        }
    });
} 
// ==========================================
// 4. LOGIKA DASBOR GLOBAL
// ==========================================
else {
    if (!actUser || !actRole) { window.location.replace("index.html"); }

    if (document.getElementById("userNameDisplay")) document.getElementById("userNameDisplay").innerText = `Login: ${actUser} (${myName})`;
    ...
    if(document.getElementById("btnLogoutOffcanvas")) { document.getElementById("btnLogoutOffcanvas").onclick = (e) => { e.preventDefault(); if(confirm("Yakin ingin keluar?")) { sessionStorage.clear(); window.location.replace("index.html"); } }; }
    ...
    if (actRole === 'admin' && isAdminPage) {
        ...
        window.readArchivedLogbook = async function(sid) {
            ...
            if(!area || !konten) return; document.getElementById("judulArsipLogbook").innerText = "Logbook Arsip: " + sid; area.classList.remove("d-none"); konten.innerHTML = '<div class="text-center small text-muted">Memuat...</div>';
            try { const logsSnap = await getDocs(query(collection(db, "logbooks"), where("schoolId", "==", sid))); let arrLogs = []; logsSnap.forEach(d => arrLogs.push({id: d.id, ...d.data()})); arrLogs.sort((a,b) => (b.waktu?.toMillis() || 0) - (a.waktu?.toMillis() || 0)); if(arrLogs.length === 0) { konten.innerHTML = '<div class="text-center text-muted mt-2">Tidak ada logbook.</div>'; return; } konten.innerHTML = arrLogs.map(d => `<div class="p-2 border rounded mb-2 bg-white shadow-sm"><div class="fw-bold text-wa border-bottom pb-1 mb-1">${d.nama.toUpperCase()} <span class="badge bg-secondary rounded-pill float-end">${d.kelas}</span></div><div class="text-dark mt-1 text-xs">📖 Materi: ${d.materi?.join(', ')}</div></div>`).join(''); } catch (error) { konten.innerHTML = '<div class="text-danger text-center">Gagal memuat arsip.</div>'; }
        };
        ...
        function renderOverviewCardsAdmin() {
            ...
            if(labelTotal) labelTotal.innerHTML = `<h5 class="fw-bold text-wa mb-1">${globalCount}</h5><span class="text-xs text-muted fw-bold">TOTAL PESERTA GLOBAL</span>`;
        }
        ...
        if(document.getElementById("btnSimpanRoadmap")) { document.getElementById("btnSimpanRoadmap").onclick = async () => { const w = document.getElementById("rmWaktu").value.trim(); const j = document.getElementById("rmJudul").value.trim(); const d = document.getElementById("rmDesc").value.trim(); if(!w || !j) return window.showModernAlert("Peringatan", "Isi formulir Roadmap."); try { await addDoc(collection(db, "roadmaps"), { waktu_target: w, judul: j, deskripsi: d, created_at: serverTimestamp() }); window.showModernAlert("Berhasil", "Roadmap disimpan!", "success"); document.getElementById("rmWaktu").value = ""; document.getElementById("rmJudul").value = ""; document.getElementById("rmDesc").value = ""; } catch(e) { window.showModernAlert("Kesalahan", e.message); } }; }
        onSnapshot(query(collection(db, "roadmaps")), (snap) => { const listOverview = document.getElementById("overviewRoadmap"); const listSistem = document.getElementById("sistemRoadmapList"); let dataRoadmap = []; snap.forEach(d => { dataRoadmap.push({id: d.id, ...d.data()}); }); dataRoadmap.sort((a,b) => (a.created_at?.toMillis() || 0) - (b.created_at?.toMillis() || 0)); if(listOverview) listOverview.innerHTML = dataRoadmap.map(r => `<li class="timeline-item"><div class="timeline-date">${r.waktu_target}</div><div class="timeline-title">${r.judul}</div><div class="timeline-desc">${r.deskripsi}</div></li>`).join(''); if(listSistem) listSistem.innerHTML = dataRoadmap.map(r => `<li class="list-group-item d-flex justify-content-between align-items-center bg-transparent border-bottom"><div><div class="fw-bold text-dark text-sm">${r.judul}</div><div class="text-muted" style="font-size:0.65rem">${r.waktu_target}</div></div><i class="bi bi-trash text-danger" style="cursor:pointer;" onclick="window.removeRoadmapAdmin('${r.id}')"></i></li>`).join(''); });
        window.removeRoadmapAdmin = async function(id) { if(confirm("Hapus peta jalan?")) await deleteDoc(doc(db, "roadmaps", id)); };

        onSnapshot(collection(db, "users"), (snap) => {
            globalAllUsers = []; const tbody = document.getElementById("listUsersTable"); if(!tbody) return; tbody.innerHTML = "";
            snap.forEach(d => { const ud = d.data(); const uid = d.id; globalAllUsers.push({id: uid, ...ud}); let btnStatus = ud.role === 'admin' ? `<span class="badge bg-secondary p-1 text-xs">Admin</span>` : `<button class="btn btn-sm bg-transparent border-0 p-0" onclick="window.ubahStatusPengguna('${uid}', '${ud.status === 'aktif' ? 'nonaktif' : 'aktif'}')"><i class="bi ${ud.status === 'aktif' ? 'bi-toggle-on text-success' : 'bi-toggle-off text-muted'} fs-4"></i></button>`; let btnHapus = ud.role === 'admin' ? `<button class="btn btn-sm btn-outline-secondary py-0 px-1 ms-1 rounded-pill" disabled><i class="bi bi-trash"></i></button>` : `<button class="btn btn-sm btn-outline-danger py-0 px-1 ms-1 rounded-pill" onclick="window.hapusPenggunaPermanen('${uid}')"><i class="bi bi-trash"></i></button>`; let btnEdit = `<button class="btn btn-sm btn-outline-warning py-0 px-1 ms-1 rounded-pill" onclick="window.siapSuntingPengguna('${uid}', '${ud.julukan}', '${ud.role}', '${ud.pin}')"><i class="bi bi-pencil"></i></button>`; tbody.innerHTML += `<tr><td class="text-start ps-2"><b>${uid}</b><br><small class="text-muted">${ud.julukan}</small></td><td class="text-uppercase fw-bold text-xs">${ud.role}<br><span class="text-danger font-monospace">${ud.pin}</span></td><td><div class="d-flex justify-content-center align-items-center gap-1">${btnStatus}${btnEdit}${btnHapus}</div></td></tr>`; }); renderMentorChecklistAdmin();
        });
        ...
        if(document.getElementById('filterHari')) document.getElementById('filterHari').addEventListener('change', renderLogbooksListAdmin); if(document.getElementById('filterKelasHistori')) document.getElementById('filterKelasHistori').addEventListener('change', renderLogbooksListAdmin);
        ...
        async function processGlobalStudentsData() {
            ...
            try {
                ...
                logbooksSnap.forEach(doc => { const data = doc.data(); if(activeIds.includes(data.schoolId) && data.dataSiswa) { data.dataSiswa.forEach(siswa => { const identitas = `${siswa.nama} (${data.kelas})`; if(!rekapSiswaGlobal[identitas]) rekapSiswaGlobal[identitas] = { vocab: '-', speak: '-', grammar: '-', prac: '-', poinTotal: 0 }; let nilaiHuruf = (siswa.nilai || "").toUpperCase(); if(['A','B','C'].includes(nilaiHuruf)) { let mStr = (data.materi || []).join(' ').toLowerCase(); if(mStr.includes('vocab')) rekapSiswaGlobal[identitas].vocab = nilaiHuruf; if(mStr.includes('speak')) rekapSiswaGlobal[identitas].speak = nilaiHuruf; if(mStr.includes('gram')) rekapSiswaGlobal[identitas].grammar = nilaiHuruf; if(mStr.includes('prac')) rekapSiswaGlobal[identitas].prac = nilaiHuruf; } if(nilaiHuruf === 'A' || nilaiHuruf === 'A+') rekapSiswaGlobal[identitas].poinTotal += 90; else if(parseInt(nilaiHuruf) > 0) rekapSiswaGlobal[identitas].poinTotal += parseInt(nilaiHuruf); if(siswa.kehadiran === 'h') totalKehadiranGlobal.h++; if(siswa.kehadiran === 'a') totalKehadiranGlobal.a++; if(siswa.kehadiran === 's') totalKehadiranGlobal.s++; if(siswa.kehadiran === 'i') totalKehadiranGlobal.i++; }); } });
                let barisHtml = ""; let nomor = 1; let arrPrestasi = []; for (const [nama, nl] of Object.entries(rekapSiswaGlobal)) { arrPrestasi.push({ nama, poin: nl.poinTotal }); barisHtml += `<tr><td class="text-muted">${nomor++}</td><td class="text-start fw-bold text-dark text-xs">${nama}</td><td class="fw-bold ${nl.vocab==='A'?'text-success':(nl.vocab==='B'?'text-primary':'text-danger')}">${nl.vocab}</td><td class="fw-bold ${nl.speak==='A'?'text-success':(nl.speak==='B'?'text-primary':'text-danger')}">${nl.speak}</td><td class="fw-bold ${nl.grammar==='A'?'text-success':(nl.grammar==='B'?'text-primary':'text-danger')}">${nl.grammar}</td><td class="fw-bold ${nl.prac==='A'?'text-success':(nl.prac==='B'?'text-primary':'text-danger')}">${nl.prac}</td></tr>`; } tabelNilai.innerHTML = barisHtml || `<tr><td colspan="6" class="text-muted">Data nilai masih kosong.</td></tr>`; labelHadir.innerHTML = `<div class="row g-2 text-center mt-2"><div class="col-6"><div class="p-2 border rounded bg-white shadow-sm"><h4 class="fw-bold text-success mb-0">${totalKehadiranGlobal.h}</h4><span class="text-muted fw-bold" style="font-size:0.65rem;">HADIR</span></div></div><div class="col-6"><div class="p-2 border rounded bg-white shadow-sm"><h4 class="fw-bold text-danger mb-0">${totalKehadiranGlobal.a}</h4><span class="text-muted fw-bold" style="font-size:0.65rem;">ALFA</span></div></div><div class="col-6"><div class="p-2 border rounded bg-white shadow-sm"><h4 class="fw-bold text-warning mb-0">${totalKehadiranGlobal.s}</h4><span class="text-muted fw-bold" style="font-size:0.65rem;">SAKIT</span></div></div><div class="col-6"><div class="p-2 border rounded bg-white shadow-sm"><h4 class="fw-bold text-info mb-0">${totalKehadiranGlobal.i}</h4><span class="text-muted fw-bold" style="font-size:0.65rem;">IZIN</span></div></div></div>`;
                arrPrestasi.sort((a,b) => b.poin - a.poin).slice(0, 10); trackPrestasi.innerHTML = arrPrestasi.map((item, idx) => `<div class="d-flex justify-content-between align-items-center p-2 border rounded mb-1 bg-light text-sm"><div class="fw-bold text-dark"><span class="badge ${idx===0?"bg-warning text-dark":"bg-wa text-white"} rounded-pill">#${idx+1}</span> ${item.nama}</div><div class="fw-bold text-success">${item.poin} Pts</div></div>`).join('') || '<div class="text-muted small text-center p-3 border rounded bg-light">Data prestasi kosong.</div>';
    ...
    if (isMentorPage) {
        ...
        function renderRiwayatLogbookMentor() {
            ...
            if(beranda) {
                ...
                }).join('') || '<div class="text-muted small border rounded p-3 bg-light text-center">Belum ada riwayat aktivitas dari mentor sebelumnya.</div>';
            }
        }
