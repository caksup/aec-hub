...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (isDirekturPage) {
        ...
        window.langsungKeSekolah = function(sid) {
            ...
            unsubLogbooks = onSnapshot(query(collection(db, "logbooks"), where("schoolId", "==", sid)), (snap) => { dataLengkap = []; snap.forEach(d => dataLengkap.push(d.data())); dataLengkap.sort((a,b)=>(b.waktu?.toMillis()||0)-(a.waktu?.toMillis()||0)); extractDaysLogbookDir(); renderLogbooksListDir(); renderTrackerDir(); calculateLeaderboardStudentsDir(); renderOverviewLogbookDirektur(); });
    ...
    if (isMentorPage) {
        ...
        function renderRiwayatLogbookMentor() {
            ...
            if(list) {
                ...
                }).join('') || `<div class="text-center text-muted small p-3 border rounded bg-light">Belum ada riwayat.</div>`;
            }
            ...
            if(beranda) {
                ...
                }).join('') || '<div class="text-muted small border rounded p-3 bg-light text-center">Belum ada riwayat aktivitas dari mentor.</div>';
            }
        }
        ...
        async function eksekusiKirimLogbookMentor() {
            ...
            if(!kelas) return window.showModernAlert("Peringatan", "Pilih kelas.");
